 ...
 */
public class SilhouetteAdapter extends CursorAdapter{
    /**
    ...
    public SilhouetteAdapter(Context context, Cursor c, int flags) {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        int finish = cursor.getInt(SilhouetteList.COL_FINISH);
        int unlock = cursor.getInt(SilhouetteList.COL_UNLOCK);
        int gold_reward = cursor.getInt(SilhouetteList.COL_GOLD_REWARD);
        int gold_req = cursor.getInt(SilhouetteList.COL_GOLD_REQUIREMENT);
        String photo = cursor.getString(SilhouetteList.COL_PHOTO);
        if(finish == 1) {
            viewHolder.statusView.setTextColor(0xff00ff66);
            viewHolder.statusView.setText("Finished");
        }
        else if(unlock == 1) {
